...
public class Waves : MonoBehaviour {
	...
	void Update() {
		...
		for(int i = 0; i < lengthOfLineRenderer; i++) {
			points[i] = new Vector3(i * 0.5f, 0.3f * Mathf.Sin(i + 2*t), 0.0f);
			points2D[i] = new Vector2(i * 0.5f, 0.3f * Mathf.Sin(i + 2*t));
		}
		effector.surfaceLevel = points[(int)Math.Ceiling(player.transform.position.x+7.3f)].y;
